...
def split_videos(meta: list[dict], train_frac: float, val_frac: float,
    ...
    def coverage(cls: str, split: str) -> int:
        return sum(1 for v in class_videos[cls] if assignment.get(v) == split)
    ...
    def claim(cls: str, split: str) -> bool:
        candidates = [v for v in class_videos[cls] if v not in assignment]
    ...
    classes_by_rarity = sorted(class_videos.keys(),
                                key=lambda c: len(class_videos[c]))
    ...
    # Pass 2: fill remaining by deficit (largest gap wins)
    remaining = [v for v in all_videos if v not in assignment]
    rng.shuffle(remaining)
    for v in remaining:
        deficit = {s: cap_left(s) for s in target}
        cand = max(deficit, key=lambda s: deficit[s])
        assignment[v] = cand
...
def assign_row_splits(meta: list[dict], train_frac: float, val_frac: float, seed: int,
                       rare_strategy: str, rare_threshold: int) -> tuple[dict[int, str], dict]:
