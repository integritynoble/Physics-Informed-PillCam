"""
Cell (b+) ACDC — temporal-only over +PI 5-channel embeddings.
==============================================================

ACDC analogue of `train_cell_b_pi.py`. Same architecture as cell (b)
ACDC; the only change is the embedding source — instead of RGB
backbone embeddings, this consumes embeddings extracted from the
+PI 5-channel input-fusion backbone (RGB + 2 cardiac prior channels).

Pre-conditions:
    - build_embedding_cache_pi_acdc.py has produced embeddings_pi/seed{seed}_embeddings.npz
    - video_index_acdc.json exists

Comparison set after run:
    cell (a) ACDC      per-frame baseline (~0.85 expected from published numbers)
    cell (b) ACDC      RGB+C2
    cell (b+) ACDC     +PI 5-ch+C2  ← this run
    cell (e+) ACDC     +PI 5-ch+C2+C3 (separate trainer)

Output:
    D:/acdc/outputs/temporal_cell_b_pi/seed{seed}/best_model.pt
    paper/nature-machine-intelligence/docs/track_b_acdc_cell_b_pi_report.md
"""
...
OUTPUT_ROOT = Path("D:/acdc/outputs/temporal_cell_b_pi")
...
def train_one_seed(seed: int, video_index: Dict, epochs: int) -> Dict:
    print(f"\n{'='*60}\nSEED = {seed} (cell b+ ACDC)\n{'='*60}")
    cache_path = EMB_DIR / f"seed{seed}_embeddings.npz"
    if not cache_path.exists():
        raise SystemExit(f"missing cache: {cache_path}. "
                         "Run build_embedding_cache_pi_acdc.py first.")
...
def main():
    ...
    print(f"[main] cell (b+) ACDC on {seeds}; source = +PI 5-ch embeddings")
    ...
    for seed in seeds:
        ...
        results.append(r)
        (REPORT_DIR / "track_b_acdc_cell_b_pi_predictions.json").write_text(
    ...
    md = []
    md.append("# Track B (ACDC): cell (b+) — temporal over +PI 5-channel embeddings\n")
    md.append("**Date:** TBD")
    md.append(f"**Seeds:** {[r['seed'] for r in results]}")
    md.append("**Source embeddings:** `D:/acdc/outputs/embeddings_pi/`")
    md.append("")
    ...
    out_path = REPORT_DIR / "track_b_acdc_cell_b_pi_report.md"
