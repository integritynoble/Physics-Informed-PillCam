"""
Cell (e) ACDC — temporal + C3 over RGB embeddings (no C1).
=============================================================

ACDC analogue of `train_cell_e.py`. Per-frame input is the
concatenation of:
    - 1280-d RGB-backbone embedding from `embeddings/seed{seed}_embeddings.npz`
    - 192-d C3 autoencoder-residual feature from `c3_features.npz`
                                                          = 1472-d

Same temporal transformer as cell (b) ACDC; same training recipe.

Pre-conditions:
    - build_embedding_cache_acdc.py has produced 6 RGB embedding caches
    - build_c3_features_acdc.py has produced c3_features.npz
    - video_index_acdc.json exists

Output:
    D:/acdc/outputs/temporal_cell_e/seed{seed}/best_model.pt
    paper/nature-machine-intelligence/docs/track_b_acdc_cell_e_report.md
"""
...
OUTPUT_ROOT = Path("D:/acdc/outputs/temporal_cell_e")
...
def main():
    ...
    print(f"[main] cell (e) ACDC on {seeds}; in_dim={EMB_DIM_IN} (1280 RGB + 192 C3)")

    if not INDEX_JSON.exists():
        raise SystemExit(f"Missing {INDEX_JSON}. Run "
                         "build_video_index_acdc.py first.")
    ...
    for seed in seeds:
        ...
        results.append(r)
        (REPORT_DIR / "track_b_acdc_cell_e_predictions.json").write_text(
    ...
    md = []
    md.append("# Track B (ACDC): cell (e) — temporal + C3 over RGB embeddings\n")
    md.append("**Date:** TBD")
    md.append(f"**Seeds:** {[r['seed'] for r in results]}")
    md.append("**Model:** transformer over concat[1280d RGB backbone + 192d C3 z-scored]")
    md.append("")
    ...
    out_path = REPORT_DIR / "track_b_acdc_cell_e_report.md"
