"""
Track B Month 2: cell (e) — temporal + C3 only (no C1)
=======================================================

Per-frame input: concat[ e_s (1280-d backbone), r_feat_s (192-d C3) ]
                                                                 = 1472-d

Isolates C3's marginal contribution to the temporal model. Together
with cell (b), cell (c), cell (d), this completes the channel
ablation:

    cell (a): per-frame, no temporal           - 0.7598 baseline
    cell (b): C2 only (temporal embeddings)    - 0.7788 (+0.019)
    cell (c): C2 + C1 (structural prior)       - 0.7774 (-0.001)
    cell (d): C2 + C1 + C3 (full)              - 0.7722 (-0.007)
    cell (e): C2 + C3 (no C1)                  - this run

If cell (e) ≥ cell (b), C3 contributes when isolated; the cell (d)
regression was C1's fault (val→test shift amplification).

If cell (e) ≈ cell (b), C3 is redundant with what the temporal
transformer extracts from embeddings.

If cell (e) < cell (b), C3 actively hurts at macro level.
"""
...
OUTPUT_ROOT = Path("D:/kvasir_capsule/outputs/temporal_cell_e")
...
def main():
    ...
    print(f"[main] cell (e) on {seeds}; in_dim={EMB_DIM_IN} (1280 emb + 192 C3)")
    ...
    for seed in seeds:
        ...
        results.append(r)
        (REPORT_DIR / "track_b_cell_e_predictions.json").write_text(
    ...
    cell_a, cell_b, cell_c, cell_d = 0.7598, 0.7788, 0.7774, 0.7722
    print(f"[main] vs cell (a) baseline 0.7598: {cs_mean - cell_a:+.4f}")
    print(f"[main] vs cell (b) C2-only  0.7788: {cs_mean - cell_b:+.4f}")
    print(f"[main] vs cell (c) C2+C1    0.7774: {cs_mean - cell_c:+.4f}")
    print(f"[main] vs cell (d) C2+C1+C3 0.7722: {cs_mean - cell_d:+.4f}")
    ...
    md = []
    md.append("# Track B Month 2: cell (e) — temporal + C3 only (no C1)\n")
    md.append("**Date:** 2026-05-07")
    md.append(f"**Seeds:** {[r['seed'] for r in results]}")
    md.append("**Model:** transformer over concat[1280d backbone + 192d C3 z-scored]")
    md.append("")
    ...
    md.append(f"vs cell (c) {cell_c}: {cs_mean - cell_c:+.4f}")
    md.append(f"vs cell (d) {cell_d}: {cs_mean - cell_d:+.4f}")
    md.append("")
    ...
    if delta_b >= 0.005:
        md.append(f"**STRONG PASS.** C3 isolated contributes {delta_b:+.4f} over "
                  f"C2-only. The cell (d) regression was C1's fault, not C3's. "
                  f"Two-channel framework (C2+C3) is the correct model.")
    elif delta_b >= 0.0:
        md.append(f"**MARGINAL.** C2+C3 ≈ C2-only ({delta_b:+.4f}). C3 is "
                  f"redundant with what the temporal transformer already extracts. "
                  f"Single-channel C2-only model is parsimonious choice.")
    else:
        md.append(f"**REGRESSION.** C2+C3 = {cs_mean:.4f} < C2-only {cell_b}. "
                  f"C3 hurts at macro level even without C1. C3 belongs in "
                  f"per-class supplementary discussion only; not in main "
                  f"architecture.")
    md.append(f"\nTotal compute: {(time.time() - t0)/60:.1f} min")

    out_path = REPORT_DIR / "track_b_cell_e_report.md"
